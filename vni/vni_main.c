...
const char man_msg3[] = "Examples:\n"
	...
	"\tethtool --set-priv-flags <dev-name> enable-vfd on v0 on "
	"b4 on b1 on\n\n"
	"2. Set VF #2 rxmode with accepting untagged packets\n"
	"\tethtool --set-priv-flags <dev-name> enable-vfd on v1 on "
	"op0 on b15 on b0 on\n\n"
	"3. Add vlan-id 1 to VF #0 and #1 (multi-step op, step 1..4 "
	"to specify 4x16-bit VF mask)\n"
	"\tethtool --set-priv-flags <dev-name> enable-vfd on op1 "
	"on op0 on s2 on\n"
	"\tethtool --set-priv-flags <dev-name> enable-vfd on op1 "
	"on op0 on s1 on s0 on\n"
	"\tethtool --set-priv-flags <dev-name> enable-vfd on op1 "
	"on op0 on s1 on\n"
	"\tethtool --set-priv-flags <dev-name> enable-vfd on op1 "
	"on op0 on s0 on b1 on b0 on\n"
	"\tethtool --set-priv-flags <dev-name> enable-vfd on op1 "
	"on op0 on b15 on b0 on\n\n"
	;
